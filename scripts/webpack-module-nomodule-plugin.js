...
const selfScript = `self.modern=1`;
const makeLoadScript = (modern, legacy) => `
addEventListener('load', function() {
  ${(modern.length > legacy.length ? modern : legacy).reduce((acc, _m, i) => `
${acc}$loadjs(${modern[i] ? `"${modern[i].attributes.src}"` : undefined}, ${legacy[i] ? `"${legacy[i].attributes.src}"` : undefined})
  `, '').trim()}
})
function $loadjs(e,d,c){c=document.createElement("script"),self.modern?(e && (c.src=e,c.type="module")):d && (c.src=d),c.src && document.head.appendChild(c)}
`;
